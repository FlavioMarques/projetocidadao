...
def run_the_app():
    ...
    if f_cidade:
        ...
        dados = busca_dados(f_cidade, conMySQL)
...
#       st.plotly_chart(fig, use_container_width=True)
    ...
    st.title('Escolha o Municipio que deseja analisar sobre Furto/Roubo de Veiculos, baseado no endereço do fato: ')
    if len(dados) > 0:
        analise = 'Qtde de dados brutos : ' + str(len(dados)) + ' ==> ' + f_cidade + ' simples amostra em 5 linhas...'
        st.text(analise)
        st.dataframe(dados.style.highlight_max(axis=0))

        data = dados
        data['DATAOCORRENCIA'] = pd.to_datetime(data['DATAOCORRENCIA'])

        df = data[['DATAOCORRENCIA', 'NUMERO_BOLETIM']].groupby('DATAOCORRENCIA').count().reset_index()
        # plotagem
        fig = px.line(df, x='DATAOCORRENCIA', y='NUMERO_BOLETIM')
        st.plotly_chart(fig, use_container_width=False)

        df7 = dados.set_index('DATAOCORRENCIA',inplace=True)
        st.write(df7)

        df8 = dados.groupby('DATAOCORRENCIA').count()
...
def busca_dados(f_cidade, conMySQL):
    try:

        consulta_sql = "SELECT * FROM 5sec.veic_ro_fu_2020 where CIDADE = %s and latitude is not null"
        #        cursor = conMySQL.cursor()
        ...
        dfz.info()
        #       dados = [res[0] for res in resultado]
        dados = dfz
    except Error as e:
        st.write("Erro ao acessar tabela MySQL", e)
    return dados
...
def mapa(nx, f_cidade):
    ...
    for name, row in df.iterrows():
        folium.Marker([row['LATITUDE'], row['LONGITUDE']],
                      popup='<b>Fatos : <p> {0} <p> {1} <p> {2}<b>'.format(row['RUBRICA'],
                      row['DATAOCORRENCIA'], row['DESCR_MARCA_VEICULO'])).add_to(marker_cluster)
